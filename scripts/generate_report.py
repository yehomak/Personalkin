...
REPORTS_DIR    = Path(__file__).parent.parent / "context" / "health" / "reports" / "weekly"
...
def _fmt_time(v):
    ...
    try:
        s = str(v)
        dt = datetime.fromisoformat(s).replace(tzinfo=timezone.utc).astimezone(LOCAL_TZ)
...
def generate(week_str=None):
    ...
    output = REPORTS_DIR / f"{file_label}.md"
    output.write_text("\n".join(lines))
